...
namespace LegacyFighter\Cabs\Contracts\Application\Task\Dynamic;
...
use LegacyFighter\Cabs\Contracts\Model\State\Dynamic\Task\TaskContractStateAssembler;

class DocumentResourceManager
{
    public function __construct(
        private DocumentHeaderRepository $documentHeaderRepository,
        private TaskContractStateAssembler $assembler,
        private UserRepository $userRepository
    ) {}

    public function createDocument(int $authorId, int $price): DocumentOperationResult
    {
        $author = $this->userRepository->getOne($authorId);

        $number = $this->generateNumber();
        $documentHeader = new DocumentHeader($author->getId(), $number, $price);
